...
async def _probe(label: str, url: str, token: str) -> dict[str, Any]:
    headers = {"Authorization": f"Bearer {token}", "User-Agent": "route-skill-remote-mcp-eval/0.3.2"}
...
def _load_cases(root: Path) -> dict[str, dict[str, Any]]:
    ...
    if len(cases) != 24:
        raise SystemExit(f"Expected exactly 24 checked-in eval cases, found {len(cases)}.")
    return cases
...
def main() -> None:
    ...
    payload["ok"] = payload["case_count"] == 24 and payload["pass_count"] == 24
