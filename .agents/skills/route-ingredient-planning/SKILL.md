...
5. Treat bounded `fallback_packs` inside the recommended arguments as transparent server-side recovery; they must not be called manually by the model. Never synthesize a second waypoint plan, generate a baseline route first, or add planned stops one at a time.
