...
4. Inspect `generation_contract` before generation. Continue only when `recommended_next_call` is present and `external_generation_call_budget` is `1`; copy its `arguments` verbatim into exactly one `route.generate_multi_point_route` call. Do not add convenience fields such as `generation_mode` or `variants`. If the call is null or the budget is `0`, do not copy candidates or option-pack waypoints by hand and do not call any generation tool. Correct a missing area/start input once or explain the reported shortfall.
5. Treat bounded `fallback_packs` and distance calibration inside the recommended arguments as transparent server-side recovery; they must not be called manually by the model. The server may spend its second internal attempt tightening distance when all ingredients were reached but distance alone missed tolerance. Never synthesize a second waypoint plan, generate a baseline route first, or add planned stops one at a time.
...
7. Report the selected ingredients and the returned `ingredient_verification`, including partial, missed, substituted, and co-satisfied roles.
