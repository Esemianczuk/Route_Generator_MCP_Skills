...
2. Continue only when the planner returns `recommended_next_call` with an external generation budget of one, then copy its arguments verbatim into exactly one `route.generate_multi_point_route` call without adding `generation_mode`, `variants`, or other fields. A null call or zero budget forbids generation; never build from candidate packs, generate a baseline first, or manually run bounded fallback packs or distance-correction retries. The server owns both bounded behaviors.
