...
## Climb Catalog and Directed Ingredients

- `route.query_climbs`: regional climb intelligence by area, radius, or bounding box. It supports eligibility/category filters, an exact `total`, bounded ordered climb rows, and extrema. Default route selection remains bicycle-eligible, route-ready, primary climbs. "Largest" orders by `gain_m`, "steepest" by `max_50m_percent`, "hardest" by the cycling-specific `fiets` score, and "longest" by `distance_m`; do not interchange those measures. Use the exact returned total for questions such as "how many Cat 4 climbs," not the number of displayed rows.
- `route.get_climb`: one-id detail and fixed directed route ingredient. Preserve its real `climb_id`, entry/start, exit/summit, ordered graph arcs, graph binding, and dataset identity. Use it for requested detail, explicit-id validation, or a prior-turn "that one"; do not fan it out across every search result.
- `route.plan_ingredient_options`: accepts `climb_needs` for delegated climb selection and `climb_ids` for explicit selections. A fixed directed climb is locked, may not reverse, and requires ordered arc traversal. The planner must fail closed when a required climb is unavailable or graph-unbound, and it owns profile-network ordering among climbs and all other ingredients. Do not replace a climb with a summit waypoint, straight-line-sort a climb chain, generate a baseline first, or mutate a generic route toward the request.
- Directed climb inclusion is proven only by the generation result's directed verification: entry precedes summit, direction passes, ordered-arc coverage meets its threshold, and graph binding passes. `route.analyze_climbs` detects climbs on an existing route but does not prove that a requested catalog climb was traversed in the required direction.
