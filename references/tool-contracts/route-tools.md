...
- `route.generate_multi_point_route`: ordered anchors, tour legs, water/fuel stop every N miles, or multi-criteria point-to-point tours. For an ingredient plan, call it exactly once only when `recommended_next_call` is non-null and `external_generation_call_budget` is one. Copy the planner-provided arguments verbatim without adding fields; the server owns bounded fallback and same-pack distance-calibration attempts and returns one final stored route. Ingredient plans default to a 5% target-distance tolerance, so a distance-only miss can be tightened without a second model-visible call. The MCP accepts string `generation_mode` and integer `variants` only as compatibility aliases for imperfect clients.
